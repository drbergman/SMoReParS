function y = my_mvnpdf(X, Mu, Sigma, input_opts)
...
y = -0.5*quadform;
if ~opts.only_use_z_scores
    y = y - logSqrtDetSigma - d*log(2*pi)/2;
end
if ~opts.return_log
    y = exp(log_val);
end
...
function default_options = defaultMyMVNPDFOptions

default_options.return_log = true; % whether to return the log of the mvnpdf; default to true because this is why I made my_mvnpdf was to be able to return this value
default_options.only_use_z_scores = false; % if true, do not include the dimension term or the determinant term; just do the z-score term
