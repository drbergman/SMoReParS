...
for i = 1:length(model_type)
    ...
    if model_type(i) ~= "von_bertalanffy"
        ...
        profile_file = sprintf("data/ProfileLikelihoods_%s_resampled_clean.mat",model_type(i));
    end
    nsamps = 3;
    [f{i},~] = testProfileSMFromABM(profile_file,nsamps,sm_par_display_names,opts);
end

%% I really don't know what this is doing here...this seems to be trying to compare the fits, not show the profiles (DRB: 2024-07-17)
...
for i = 1:3
    ...
    ax(i) = gca;
    copyobj(f{i}(1).Children(i).Children(2:3),ax(i))
    for j = 1:3
