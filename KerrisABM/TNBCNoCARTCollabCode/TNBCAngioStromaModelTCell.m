%TNBC Angio-Stroma Model
%Michael Ventoso SPROJ
%T-Cell Invasion
%Modified Version: Kerri-Ann Norton
%Reducing and adapting to CAR-T Therapy

%Version 10.1Short
%Last revision: Kerri - 4/23/21
clear all
close all

%This version changes CAgrid.Agent to Agentmat
%Each cell now has its own symrate and Divrate

%Reductions
%No Fibroblasts
%No Macrophages
%Testing tumor growth with only stem cells or starting with stem cells

mkdir('Data')%data will contain sub-folders for each trial AA/AB etc. BSRI19
...
starttime = clock;
%[AA,AB,BB] = meshgrid(linspace(0.01, 0.05, 5), [1],[1]);
exType = 'Binary'; %Gradated or binary
[AA,AB,BB] = meshgrid([0], [1500], linspace(2, 6, 5));% AA is antigen non-presenting cells, AB is time of CART cell introduction BSRI 21, BB is number of trials
%loop searching a plane of area AB*AA (Square plane)
for loop = 1:numel(AB)
...
%         (RandStream('mt19937ar','seed',2.074686e+05)); %2)); %Make sure random is truly random
    ...
    if strcmp(exType,'Gradated')
        ...
        cartHetMean = AA(loop)/100;
        cartHetStd = .1;
    elseif strcmp(exType, 'Binary')
        binaryHeterogeniety = true;
        AntigenHetPercentExpressed = AA(loop)/100;%.9; % the fraction of Cancer cells that express the CAA for car T cells
    end
    ...
    DivLimit = 12*ones(100,1); %Used to be 12 - still is because not based on time
    DivRate = 0.125*ones(100,1);
      ...
      %  hyploc = zeros(3600,3);
        ...
        for jj = 1:numberofcells %proliferate all cells so far but not the new ones
            ...
            if ~isempty(emptypos)
                ...
                if min(thedistm) <= 10
                    ...
                    elseif CellState(jr) == 2
                        if HypCell(jr) == 0
                            ...
                            DivRate(jr) = promultm*0.4375;
                        else
                            %hypoxia decrease by 1/2
                            DivRate(jr) = promultm*0.5*0.4375;
                        end
                    end
                ...
                if startprolif == 1 %CHANGED V4.1 %Produce Progen Cell
                    ...
                    QState(empspace) = 1;
                    DivLimit(empspace) = 12;
                    DivRate(empspace) = 0.125;%0.5;
        ...
        if mod(time, 10) == 0
          ...
          %  hyploc(hypcount:end, :) = [];
            ...
            name4 =strcat('State','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    ...
    namedataA =strcat('NumberofCells','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(namedataA, cellsovertime, 'delimiter', '\t')
    
    namedataD =strcat('NumberofDeath','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(namedataD, deathovertime, 'delimiter', '\t')
    
    namedataS =strcat('NumberofStems','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(namedataS, stemsovertime, 'delimiter', '\t')
    
    namedataH =strcat('NumberofHypoxic','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(namedataH, hypoxiaovertime, 'delimiter', '\t')
    
    namedataH1 =strcat('HypCell','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(namedataH1, HypCell, 'delimiter', '\t')
    
    namedataM =strcat('NumberofMacro','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(namedataM, macroovertime, 'delimiter', '\t')
    
    namedataC =strcat('NumberofCCR5','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(namedataC, CCR5overtime, 'delimiter', '\t')
    
    namedataK =strcat('NumberofCART','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(namedataK, cartovertime, 'delimiter', '\t')
    
    namedataP =strcat('NumberofProlif','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(namedataP, prolifovertime, 'delimiter', '\t')
    
    namedataKills =strcat('NumberofKills','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(namedataKills, killsovertime, 'delimiter', '\t')
    
    namedataVasc =strcat('NumberofVasc','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    ...
    name3 =strcat('XYZ','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(name3, XYZ, 'delimiter', '\t')
    
    name3b =strcat('XYZh','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(name3b, XYZh, 'delimiter', '\t')
    
    name4 =strcat('Stateh','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    dlmwrite(name4, CellState, 'delimiter', '\t')
    
    %Also save capillaries
    SaveAddress2 = strcat('CapListp', '_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time));
    CapMatrix = CapillaryList;
    %saveCapMatrix(SaveAddress2, CapMatrix);
    save(SaveAddress2, 'CapMatrix');
    
    name2 =strcat('CellMatrix','_STEM','_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
    %saveAgentmat(name2, Agentmat);
    save( name2, 'Agentmat');
    
    namedata =strcat('rundata',num2str(runnum),'_CART',num2str(AA(loop)),'_Trial',num2str(BB(loop)),'_t',num2str(time),'.txt');
