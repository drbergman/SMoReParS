...
param1 = 'pdiv '; %antigen expression
param2 = 'dlim ';
...
colors = [238,204,102;102,153,204;153,68,85;0,68,136;0,0,0; 255,0,0;255,255,0;0,255,0;0,255,255]./255;
%Line style 
%liness2 = [];
liness = {'-'; '--'; ':'; '.-'};
...
ignore = ["State","Macro","Vasc","HypCell","AntigenExp"];
...
if plotAvg
    ...
    for p = 1:length(data(1,1,1,:,1))
        ...
        for AA = 1:length(aa)
            for AB = 1:length(ab)
                plot(squeeze(meanData(AA,AB,:,p,:)),'color',colors(AA,:),'LineStyle', liness{AB,:}, 'LineWidth',2);
