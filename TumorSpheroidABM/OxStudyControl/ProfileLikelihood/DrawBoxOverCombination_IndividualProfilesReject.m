...
f.Name = "DifferntComboInfoBox_IndividualProfilesReject";
ax = gca;
...
hold on
abm_par_ind = 406;
f.Name = f.Name + sprintf("_%d",abm_par_ind);
...
for i = 1:3
    ...
    % c{3} = [zz{i}(I1),P1(3)];
    % [~,order] = sort(c{i});
    % plot3(c{1}(order),c{2}(order),c{3}(order),"LineStyle",line_style_out_box,"LineWidth",1,"Color",colors(i,:));
    % 
...
%%
margin = struct("left",0.18,"right",.08,"top",0,"bottom",0.18);
