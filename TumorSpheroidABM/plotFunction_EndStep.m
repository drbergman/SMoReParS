...
if M.plot_pars.plot_location
    if M.setup.ndims == 3
        M.fig.scatter_plots(1).XData = M.tumor(:,M.I.subs(1))-M.grid.center(1);
        M.fig.scatter_plots(1).YData = M.tumor(:,M.I.subs(2))-M.grid.center(2);
        M.fig.scatter_plots(1).ZData = M.tumor(:,M.I.subs(3))-M.grid.center(3);
...
M.fig.subpop_plots(1).YData(end+1) = M.tracked.phase_cell_hours(M.i,1)/(M.dt*M.tracked.NT(M.i-1));
M.fig.subpop_plots(2).XData(end+1) = M.t;
M.fig.subpop_plots(2).YData(end+1) = M.tracked.phase_cell_hours(M.i,2)/(M.dt*M.tracked.NT(M.i-1));
M.fig.subpop_plots(3).XData(end+1) = M.t;
M.fig.subpop_plots(3).YData(end+1) = M.tracked.phase_cell_hours(M.i,3)/(M.dt*M.tracked.NT(M.i-1));
