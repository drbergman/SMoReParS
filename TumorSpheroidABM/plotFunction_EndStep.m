...
M.fig.subpop_plots(1).YData(end+1) = M.tracked.tumor_types(M.i,1)/M.tracked.NT(M.i-1);
M.fig.subpop_plots(2).XData(end+1) = M.t;
M.fig.subpop_plots(2).YData(end+1) = M.tracked.tumor_types(M.i,2)/M.tracked.NT(M.i-1);
M.fig.subpop_plots(3).XData(end+1) = M.t;
M.fig.subpop_plots(3).YData(end+1) = M.tracked.tumor_types(M.i,3)/M.tracked.NT(M.i-1);
