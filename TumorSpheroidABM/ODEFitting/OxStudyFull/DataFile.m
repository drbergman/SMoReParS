...
sigma_count(:,1) = [0.099 0.193 0.207 0.298 0.168 0.364]';  % thousands of cells

% Cell Cycle Distribution (none given)
state2_prop(:,1) = NaN(size(tt));
sigma_state2_prop(:,1) = NaN(size(tt));

%% 0.75 uM Oxaliplatin
count(:,2) = [0.899 1.077 1.658 2.059 2.584 3.387]';  % thousands of cells
sigma_count(:,2) = [0.104 0.113 0.105 0.370 0.570 0.323]';  % thousands of cells

% Cell Cycle Distribution
state2_prop(:,2) = [ 9.629 19.092 17.648 15.006 13.051 11.774]' / 100; % proportion of cells in G2/M
sigma_state2_prop(:,2) = [2.3500  1.2090  1.3950  1.2420  1.1640  1.6030]' / 100; % standard error in proportion of cells in G2/M

%% 7.55 uM Oxaliplatin
count(:,3) = [0.899 0.960 1.290 1.226 1.254 1.126]';  % thousands of cells
sigma_count(:,3) = [0.098 0.081 0.151 0.248 0.043 0.120]';  % thousands of cells

% Cell Cycle Distribution
state2_prop(:,3) = [ 9.957 17.766 17.256 19.218 21.689 21.475]' / 100; % proportion of cells in G2/M
sigma_state2_prop(:,3) = [1.2550  1.8070  1.8240  2.3920  2.9690  4.4850]' / 100; % standard error in proportion of cells in G2/M

%% normalize counts and error to start at 100
temp = 100./count(1,:);
count = count.*temp;
sigma_count = sigma_count.*temp;
