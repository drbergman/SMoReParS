...
%                     M.tracked.phase_cell_hours(M.i,[2,3]) = M.tracked.phase_cell_hours(M.i,[2,3]) + (min(M.pars.mitosis_duration,M.dt-M.tumor(j,M.I.proliferation_timer))/M.dt) * [-1,1];
                    M.tracked.phase_cell_hours(M.i,[2,3]) = M.tracked.phase_cell_hours(M.i,[2,3]) + M.pars.mitosis_duration * [-1,1]; % if the mitosis duration is longer than dt, this will result in negative looking values, but otherwise I have to try to keep track of this info into the next step, which would be tedious
