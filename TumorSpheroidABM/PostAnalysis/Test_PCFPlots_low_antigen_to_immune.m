...
nc = 2;
ax = gobjects(2,2);
f(end+1) = figure("Name","pcf_average_low_antigen_to_immune");
...
%% arg max patch plots
f(end+1) = figure("Name","pcf_argmax_low_antigen_to_immune");
ax = gca;
hold on;
pp = gobjects(2,2);
ll = gobjects(2,2);
...
title(["Radius of Maximal Clustering","of CTLs near LA Cells"])
set(ax,'FontSize',16)
L = legend(ll(:),["No effect","R (recruit)","C (cytotoxic)","R+C"],"Location","best");
L.Title.String = ["FGFR3 Effect","on CTLs"];
L.Position = [0.1946 0.3869 0.2144 0.4200];
f(end).Position = [1166 883 569 275];

%% r in expectation patch plots
f(end+1) = figure("Name","pcf_rint_low_antigen_to_immune");
ax = gca;
hold on;
pp = gobjects(2,2);
ll = gobjects(2,2);
...
xlabel('Time (days)')
ylabel('r (cell widths)')
title(["\int rg(r)dr / \int g(r)dr","of CTLs near LA Cells"])
set(ax,'FontSize',16)
L = legend(ll(:),["No effect","R (recruit)","C (cytotoxic)","R+C"],"Location","best");
L.Title.String = ["FGFR3 Effect","on CTLs"];
