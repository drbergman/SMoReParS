...
load(sprintf("../data/%s/pcf.mat",cohort_name))
...
nc = 2;
ax = gobjects(2,2);
f(end+1) = figure("Name","pcf_average_default");
...
%% arg max patch plots
f(end+1) = figure("Name","pcf_argmax_default");
ax = gca;
hold on;
pp = gobjects(2,2);
ll = gobjects(2,2);
...
title(["Radius of Maximal Clustering","of CTLs near Tumor Cells"])
set(ax,'FontSize',16)
...
f(end+1) = figure("Name","pcf_rint_default");
ax = gca;
hold on;
pp = gobjects(2,2);
ll = gobjects(2,2);
...
xlabel('Time (days)')
ylabel('r (cell widths)')
title(["\int rg(r)dr / \int g(r)dr","of CTLs near Tumor Cells"])
set(ax,'FontSize',16)
L = legend(ll(:),["No effect","R (recruit)","C (cytotoxic)","R+C"],"Location","best");
L.Title.String = ["FGFR3 Effect","on CTLs"];
