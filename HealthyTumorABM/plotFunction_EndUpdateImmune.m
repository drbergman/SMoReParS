function M = plotFunction_EndUpdateImmune(M,in) % storing order below is the ONLY reason I need to output M
...
if ~isempty(in.p_matrix_imm)
    ...
    if ~isequal(order,M.fig.imm_prob_order)
        legend(M.fig.ax(M.fig.tumor_prob_ind),...
...
M.fig.receptor_plots(2).YData(end+1) = mean(M.pd1_signaling_on_alive_immunes(M.pd1_signaling_on_alive_immunes~=0)) / M.immune_pars.deactivation_ec50_pd1;